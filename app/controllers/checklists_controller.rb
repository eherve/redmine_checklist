  ...
  def done
    ...
    if @checklist_item.save
      ...
      if (Setting.issue_done_ratio == "issue_field") && RedmineChecklists.settings[:issue_done_ratio]
        ...
        total_unspecified = 0
        done_unspecified = 0
        @checklist_item.issue.checklists.each {
          |c|
          if c.ratio != nil then
            specified_rate += c.ratio
            if c.is_done then done_specified += c.ratio end
          else
            total_unspecified += 1
            if c.is_done then done_unspecified += 1 end
          end
        }
        if done_unspecified > 0 && done_unspecified == total_unspecified then
          done_unspecified = 100 - specified_rate
        elsif total_unspecified > 0
